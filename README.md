...
Код проекта полностью документирован с использованием **Doxygen**. Все классы, методы и ключевые функции снабжены комментариями в формате Doxygen, что позволяет автоматически генерировать справочную документацию.

![Скриншот документации Doxygen](Документация_Doxygen.png)

### Содержание документации

- **Классы:** `Cache`, `Validator`, `Logger`, `TelemetryServiceImpl`
- **Методы:** `GetParameter`, `SetParameter`, `Log`, `LogError`, `ValidateName`, `ValidateValue`, `ValidateRequestId`, `IsRequestIdUsed`, `MarkRequestIdUsed`, `Set`, `Get`, `Exists`
- **Файлы:** Все `.h` и `.cpp` файлы с описанием назначения
- **Примеры использования:** Демонстрация работы основных компонентов

### Как сгенерировать документацию

1. Установите Doxygen: https://www.doxygen.nl/download.html
2. В корне проекта выполните: **doxygen**
3. Откройте docs/html/index.html в браузере (команда: **start docs/html/index.html**).
...
Проект создан в рамках тестового задания для компании **"Д4 Технологии"** на реализацию gRPC сервиса
